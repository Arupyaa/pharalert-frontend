...
//       <section className="relative min-h-screen flex w-full items-start px-4 py-12 overflow-visible">
//         {/* Backgrounds */}
      ...
      >
        {loading ? (
          <div className="flex flex-col items-center gap-2 py-10">
            <svg className="w-5 h-5 animate-spin" style={{ color: "var(--brand-primary)" }} fill="none" viewBox="0 0 24 24">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
            </svg>
            <p className="text-xs" style={{ color: "var(--text-muted)" }}>Loading medications…</p>
          </div>
        ) : filtered.length === 0 ? (
          <div className="py-10 text-center text-sm" style={{ color: "var(--text-muted)" }}>
            {search ? "No results found." : "No unlinked medications available."}
          </div>
        ) : (
          <div className="max-h-[280px] overflow-y-auto divide-y" style={{ borderColor: "var(--border-gray)" }}>
            {filtered.map((med) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0" onClick={() => onToggle(med.id)}>
          ...
          <div key={label} className="flex items-center gap-1.5 flex-1">
            <div className="flex flex-col items-center gap-1 flex-1">
              <div
                ...
                style={{
                  ...
                  border: done || active ? "none" : "1.5px solid var(--border-gray)",
                }}
              >
                {done ? (
            ...
            {i < steps.length - 1 && (
              <div
                ...
                style={{ background: step > idx ? "var(--brand-primary)" : "var(--border-gray)" }}
              />
            )}
          </div>
        );
...
function RoleForm({ activeRole, onSuccess, onError }) {
  ...
  async function handleMedsContinue() {
    ...
    const finalPayload = {
      ...pendingPayload,
      ...(selectedMedIds.length > 0 ? { suggestedMedicationIds: selectedMedIds } : {}),
    };
    setCompanyStep(3);
  ...
  if (activeRole === "companies" && companyStep === 3) {
    return (
      <div className="flex flex-col items-center gap-3 py-10">
        ...
        <p className="text-sm font-medium" style={{ color: "var(--text-muted)" }}>Submitting your registration…</p>
      </div>
    );
  }
            ...
            <div className="grid lg:grid-cols-2 gap-6 lg:gap-10 items-start">
              ...
              <div className="flex justify-center lg:justify-end">
                <div
                  ...
                  style={{ boxShadow: "var(--shadow-card)", maxHeight: "calc(100vh - 80px)" }}
                >
                  {successInfo ? (
                    <div className="text-center py-6">
                      <div
