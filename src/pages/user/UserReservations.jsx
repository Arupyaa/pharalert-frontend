  ...
  return (
    <div className="p-6" style={{ background: "var(--color-bg-subtle)", minHeight: "100%" }}>
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm" style={{ color: "var(--text-muted)" }}>View and manage your medicine reservations</p>
      </div>
      <div className="rounded-2xl p-12 flex flex-col items-center justify-center text-center" style={{ background: "var(--bg-neutral)", border: "1px dashed var(--color-primary-25)" }}>
        ...
        <p className="font-semibold" style={{ color: "var(--text-heading)" }}>Upgrade your account to create reservations</p>
        <p className="text-sm mt-1" style={{ color: "var(--text-muted)" }}>Paid plan users can reserve medicines at nearby pharmacies</p>
      </div>
    </div>
  );
}
...
function PaidReservationsPage() {
  ...
  const {
    ...
    queryFn: () =>
      api.get("/medications", { params: { search: debouncedSearch } }).then((res) => res.data.data || []),
  ...
  const updateQuantity = useCallback((id, value) => {
    ...
    if (!isNaN(qty) && qty > 0) {
      setItems((prev) =>
        prev.map((item) => (item.medicationId === id ? { ...item, quantity: qty } : item))
      );
    }
  }, []);
    ...
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        <div className="lg:col-span-2">
          <div className="rounded-2xl p-6" style={{ background: "var(--bg-neutral)", border: "1px solid var(--border-gray)", boxShadow: "0 1px 12px var(--color-shadow-4)" }}>
            ...
            <div className="relative w-full" ref={dropdownRef}>
              <div className="flex items-center gap-2 bg-white border border-gray-200 rounded-xl px-4 py-2.5 shadow-sm transition focus-within:ring-2 focus-within:ring-green-500">
                ...
                <input
                  type="text"
                  value={medInput}
                  onChange={(e) => { setMedInput(e.target.value); setShowDropdown(true); }}
                  onFocus={() => setShowDropdown(true)}
                  placeholder="Search medicine by name..."
                  className="flex-1 bg-transparent outline-none text-sm border-none"
                />
                {medsLoading && <Loader size={16} className="animate-spin text-gray-400 shrink-0" />}
              </div>

              {showDropdown && debouncedSearch?.length >= 1 && medOptions.length > 0 && (
                <div className="absolute z-20 mt-1 w-full bg-white border border-gray-200 rounded-xl shadow-xl max-h-60 overflow-y-auto" style={{ boxShadow: "0 4px 24px var(--color-shadow-4)" }}>
                  {medOptions.map((med) => (
                    <button
                      key={med.id}
                      type="button"
                    ...
                    >
                      <div className="min-w-0">
                        <span className="font-medium" style={{ color: "var(--text-heading)" }}>{med.brandName}</span>
                        <span className="ml-2 text-xs" style={{ color: "var(--text-muted)" }}>{med.genericName}</span>
                        <span className="ml-2 text-xs font-medium" style={{ color: "var(--brand-primary)" }}>
              ...
              {showDropdown && debouncedSearch?.length >= 1 && !medsLoading && medOptions.length === 0 && (
                <div className="absolute z-20 mt-1 w-full bg-white border border-gray-200 rounded-xl shadow-xl p-4 text-center text-sm" style={{ color: "var(--text-muted)", boxShadow: "0 4px 24px var(--color-shadow-4)" }}>
                  No medicines found
                </div>
              )}
            </div>
            ...
            {items.length > 0 && (
              <div className="mt-4 space-y-2">
                {items.map((item) => (
                  <div key={item.medicationId} className="flex items-center gap-3 p-3 rounded-xl bg-white border border-gray-100">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="font-medium text-sm" style={{ color: "var(--text-heading)" }}>{item.brandName}</span>
                        {item.manufacturer && <span className="text-xs" style={{ color: "var(--text-muted)" }}>{item.manufacturer}</span>}
                      </div>
                      <div className="flex items-center gap-3 mt-1">
                        <span className="text-xs" style={{ color: "var(--text-muted)" }}>{item.genericName}</span>
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        min="1"
                        value={item.quantity}
                        onChange={(e) => updateQuantity(item.medicationId, e.target.value)}
                        className="w-16 px-2 py-1.5 text-sm border border-gray-200 rounded-lg bg-white outline-none focus:ring-2 focus:ring-green-500 text-center"
                      />
                      <span className="text-sm font-semibold min-w-[70px] text-right" style={{ color: "var(--text-heading)" }}>
        ...
        <div className="space-y-4">
          <div className="rounded-2xl p-5" style={{ background: "var(--bg-neutral)", border: "1px solid var(--border-gray)", boxShadow: "0 1px 12px var(--color-shadow-4)" }}>
            <h3 className="text-sm font-semibold mb-3" style={{ color: "var(--text-heading)" }}>Reservation Stats</h3>
            <div className="space-y-3">
              <div className="flex items-center justify-between p-3 rounded-xl" style={{ background: "#f8fafc" }}>
                ...
                <span className="text-sm font-semibold" style={{ color: "var(--text-heading)" }}>{stats.total}</span>
              </div>
              ...
              <div className="flex items-center justify-between p-3 rounded-xl" style={{ background: "#f0fdf4" }}>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm" style={{ color: "var(--text-muted)" }}>Delivered</span>
                </div>
                <span className="text-sm font-semibold text-green-600">{stats.delivered}</span>
              </div>
            </div>
          </div>

          <div className="rounded-2xl p-5" style={{ background: "var(--bg-neutral)", border: "1px solid var(--border-gray)", boxShadow: "0 1px 12px var(--color-shadow-4)" }}>
            <h3 className="text-sm font-semibold mb-3" style={{ color: "var(--text-heading)" }}>Quick Tips</h3>
      ...
      <div className="mb-4 flex items-center gap-2">
        <h2 className="text-lg font-semibold" style={{ color: "var(--text-heading)" }}>Your Reservations</h2>
      ...
      {!isLoading && !fetchError && reservations.length === 0 && (
        <div className="rounded-2xl p-12 flex flex-col items-center justify-center text-center" style={{ background: "var(--bg-neutral)", border: "1px dashed var(--color-primary-25)" }}>
          <CalendarCheck size={40} className="mb-3" style={{ color: "var(--text-muted)" }} />
          <p className="font-semibold" style={{ color: "var(--text-heading)" }}>No reservations found</p>
          <p className="text-sm mt-1" style={{ color: "var(--text-muted)" }}>
            {statusFilter === "all" ? "Create your first reservation above" : `No ${statusFilter} reservations yet`}
          </p>
        </div>
      )}
    ...
    <div className="flex items-center gap-1">
      {steps.map((step, idx) => (
        <div key={step.label} className="flex items-center">
          ...
          {idx < steps.length - 1 && (
            <div className={`w-8 h-0.5 ${steps[idx + 1].done ? "bg-green-500" : "bg-gray-200"}`} />
          )}
        </div>
      ))}
    </div>
      ...
      {!isExpanded && (
        <div className="px-5 py-3">
          <div className="flex items-center gap-2 text-xs" style={{ color: "var(--text-muted)" }}>
            ...
            <span>Deliver by {new Date(reservation.deliveryDate).toLocaleDateString()}</span>
          </div>
        </div>
      )}
          ...
          <div className="border-t border-gray-200 pt-3 space-y-3">
                  ...
                  >
                    ...
                    <div className="text-right">
                      <span className="text-sm" style={{ color: "var(--text-heading)" }}>
                        {item.quantity} x {Number(item.unitPrice).toFixed(2)} EGP
                      </span>
                      <div className="text-xs font-medium" style={{ color: "var(--brand-primary)" }}>
            ...
            <div className="flex items-center justify-between pt-2 border-t border-gray-200">
              <div className="space-y-1 text-xs" style={{ color: "var(--text-muted)" }}>
                ...
                <div>Deliver by: {new Date(reservation.deliveryDate).toLocaleDateString()}</div>
              </div>
              <div className="text-right">
