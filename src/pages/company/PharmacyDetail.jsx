...
export default function PharmacyDetail() {
  ...
  const fetchPharmacy = useCallback(async (filters = {}) => {
    ...
    return (
      <div className="bg-neutral-secondary min-h-screen p-4 sm:p-6">
        ...
        <div className="rounded-2xl p-10 flex flex-col items-center justify-center text-center bg-neutral-main border border-border-primary">
          ...
          <button onClick={() => fetchPharmacy({})} className="mt-3 text-sm text-brand-primary underline underline-offset-2">
            Try again
          </button>
        </div>
      </div>
    ...
    <div className="bg-neutral-secondary min-h-screen p-4 sm:p-6">
      ...
      <div className="rounded-2xl p-6 bg-neutral-main border border-border-primary shadow-sm mb-6">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            <h1 className="text-2xl font-bold text-heading">{record.pharmacyName}</h1>
        ...
        <div className="flex flex-wrap gap-4 mt-4 pt-4 border-t border-border-primary">
          <div className="text-center min-w-[80px]">
            <p className="text-xl font-bold text-heading">{record.noOfMedsStocked || 0}</p>
            <p className="text-xs text-muted">Stocked</p>
          </div>
          <div className="text-center min-w-[80px]">
            <p className="text-xl font-bold text-amber-600">{record.noOfMedsInShortage || 0}</p>
            <p className="text-xs text-muted">Shortage</p>
          </div>
          <div className="text-center min-w-[80px]">
            <p className="text-xl font-bold text-red-600">{record.noOfMedsInCritical || 0}</p>
        ...
        <div className="flex flex-wrap items-end gap-3">
          {categories.length > 0 && (
            <div className="flex-1 min-w-[140px] max-w-[200px]">
              <label className="block text-xs font-semibold mb-1 uppercase tracking-wider text-muted">Category</label>
          ...
          {hasFilters && (
            <button type="button" onClick={() => { setCategoryId(""); setFromDate(""); setToDate(""); }}
      ...
      {medications.length === 0 ? (
        <div className="rounded-2xl p-10 flex flex-col items-center justify-center text-center bg-neutral-main border border-border-primary">
          <p className="text-muted text-sm">No medication data available for this pharmacy.</p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm text-left text-paragraph min-w-[700px]">
            <thead>
              <tr className="border-b border-border-primary bg-neutral-tertiary">
                {MED_HEADERS.map((h) => (
                  <th key={h.key} className="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-muted whitespace-nowrap">
                    {h.label}
                  </th>
                ))}
              </tr>
              ...
              {medications.map((med, idx) => (
                <tr key={idx} className="border-b border-border-primary bg-neutral-main hover:bg-neutral-secondary transition-colors">
                  {MED_HEADERS.map((h) => {
                    ...
                    return (
                      <td key={h.key} className="px-4 py-3 text-paragraph whitespace-nowrap">
                        {value ?? "—"}
                      </td>
                    );
                  })}
