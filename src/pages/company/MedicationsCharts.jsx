...
export default function MedicationsCharts() {
  ...
  const fetchChart = useCallback(async () => {
    ...
    try {
      ...
      const { data: res } = await api.get("/company/analytics/medications/charts", { params });
      setData(res?.data ?? []);
    } catch {
      setData([]);
    } finally {
  ...
  return (
    <div>
      <div className="mb-5">
        <h1 className="text-xl sm:text-2xl font-bold text-heading tracking-tight">Medications Charts</h1>
        <p className="text-muted text-sm mt-0.5">Medication demand trends over time</p>
      </div>
